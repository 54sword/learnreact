...
class Settings extends Component {
  ...
  render() {
    ...
    // 昵称
    let resetNickname = <Link className="arrow" to="/settings/reset-nickname">修改名字 <span className="right">{user.nickname}</span></Link>
    ...
    // 重置邮箱
    if (user.email) {
      email = (<Link className="arrow" to="/settings/reset-email">修改邮箱
      ...
      <div>
        ...
        <div className="container">
          ...
          <div className="list">
            ...
            {resetNickname}
            <Link className="arrow" to="/settings/reset-gender">
              性别 <span className="right">{user.gender == 1 ? '男' : '女'}</span>
            </Link>
            <Link className="arrow" to="/settings/reset-brief">
          ...
          <div className="list">
            {email}
            {user.email ? <Link className="arrow" to="/settings/reset-password">修改密码</Link> : null}
          </div>
